...
namespace AssignmentDay23
{
    ...
    public class UC9 : AddressBook
    {
        ...
        public List<UC9> stateList;
        public List<UC9> cityList;
        public List<UC9> people;
        ...
        public UC9(string firstName, string lastName, string address, string city, string state, string zip, string phoneNumber, string email)
        {
        ...
        public void Customer(string firstName, string lastName, string phoneNum, string address, string city, string state, string zipCode, string emailId)
        {
            int contact = 0;
            UC9 person = new UC9(firstName, lastName, phoneNum, address, city, state, zipCode, emailId);
            if (contact == 0)
            {
                ...
                {
                    List<UC9> existing = State[state];
                    existing.Add(person);

                }
                else
                {
                    stateList = new List<UC9>();
                ...
                {
                    List<UC9> existing = City[city];
                    existing.Add(person);

                }
                else
                {
                    cityList = new List<UC9>();
                    cityList.Add(person);
            ...
            {
                UC9 addressBookSystems = people.Find(x => x.firstName.Equals(firstName));
                if (addressBookSystems == null)
                {
                    person = new UC9(firstName, lastName, address, city, state, zipCode, phoneNum, emailId);
                    people.Add(person);
                    if (State.ContainsKey(state))
                    {
                        List<UC9> existing = State[state];
        ...
        public void RemovePeople()
        {
            ...
            UC9 person = people.FirstOrDefault(x => x.firstName.ToUpper() == firstName.ToUpper());
            if (person == null)
            {
        ...
        public static void StoreCityList(string key, List<UC9> cityList, string city)
        {
            List<UC9> CityList = cityList.FindAll(a => a.city.ToLower() == city);
        ...
        public static void StoreStateList(string key, List<UC9> stateList, string state)
        {
            List<UC9> StateList = stateList.FindAll(x => x.state.ToLower() == state);
